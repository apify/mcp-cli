...
- OAuth callback port range for the hosted CIMD narrowed from 13316–13325 to 13316–13320 (5 ports), and `localhost` variants dropped from the CIMD's `redirect_uris` in favor of `127.0.0.1` only (per RFC 8252 §8.3, which recommends the IP literal to avoid DNS resolution ambiguity)
